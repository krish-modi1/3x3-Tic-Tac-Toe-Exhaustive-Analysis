...
def generate_final_analysis_no_weights(position_file):
    ...
    # --- 2. Calculate Weights and Centered Probs (RECURSIVE) ---
    state_adj_prob_map = {}
    ...
    print("Running Recursive Adjustment...")
    for layer in layers:
        ...
        for parent, group in parent_groups:
            ...
            indices = group.index
            fuzzy_plot_df.loc[indices, 'weight'] = w
            fuzzy_plot_df.loc[indices, 'parent_adj_prob'] = p_parent_adj
            
            new_probs = group['px'] - (w * p_parent_adj)
    ...
    output_csv = 'position_4_weighted_difference.csv'
    ...
    for idx, layer in enumerate(range(1, 10)):
        ...
        if len(layer_data) > 0:
            ...
            ax.set_title(f'Layer {layer} (Original)', fontsize=12, fontweight='bold')
            ax.set_ylim(common_ylim) # Enforce consistent scale
        else:
    ...
    plt.suptitle('Original Position 4 Analysis', fontsize=16, fontweight='bold')
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig('position_4_original.png', dpi=150)
    plt.close()
    
    # --- 4. Generate PNG 2: Methodology Analysis (Recursive) ---
    print("Generating Methodology Plot (Recursive)...")
    ...
    for idx, layer in enumerate(range(1, 10)):
        ...
        if len(layer_data) > 0:
            ...
            ax.set_title(f'Layer {layer} (Weighted Difference)', fontsize=12, fontweight='bold')
            ax.set_ylim(common_ylim) # Enforce consistent scale
        else:
    ...
    plt.suptitle('Weighted Difference', fontsize=16, fontweight='bold')
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig('position_4_weighted_difference.png', dpi=150)
    plt.close()

# Execute
generate_final_analysis_no_weights('position_4_probabilities.csv')